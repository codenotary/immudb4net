...
public class KV
{
    ...
    byte[] Digest_v1()
    {
        ...
        {
            WriteWithBigEndian(bw, (short)mdLen);
            if (mdLen > 0)
            {
                WriteWithBigEndian(bw, mdbs);
            }
            WriteWithBigEndian(bw, (short)Key.Length);
            WriteWithBigEndian(bw, Key);
            WriteWithBigEndian(bw, CryptoUtils.Sha256Sum(Value));
        }
