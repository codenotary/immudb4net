...
public static class CryptoUtils
{
    ...
    */
            ...
            byte[] sourceAlh, byte[] targetAlh)
    {
        ...
        // Validate linear proof lengths.
        // The AHT tree is built synchronously which means that the linear proof
        // length must be at most 1. We validate that for both the source and target
        // transaction.
        if ((proof.TargetTxHeader.Id < proof.TargetTxHeader.BlTxId) ||
            (proof.TargetTxHeader.Id > proof.TargetTxHeader.BlTxId + 1))
        {
            return false;
        }

        if ((proof.SourceTxHeader.Id < proof.SourceTxHeader.BlTxId) ||
            (proof.SourceTxHeader.Id > proof.SourceTxHeader.BlTxId + 1))
        {
            return false;
        }
